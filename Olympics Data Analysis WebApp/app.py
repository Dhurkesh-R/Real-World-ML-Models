...
st.sidebar.image("https://upload.wikimedia.org/wikipedia/commons/a/a7/Olympic_flag.svg", width=300)
...
if user_menu == "Medal Tally":
    ...
    years, country = helper.fetch_country_year_list(df)
    ...
    if selected_year == 'Overall' and selected_country != 'Overall':
        st.title(f"Overall Medal Tally in {selected_country}")
    if selected_year != 'Overall' and selected_country == 'Overall':
...
if user_menu == "Overall Analysis":
    ...
    st.title("Participating Countries Over Time")
    country_over_time = helper.data_over_time(df, 'region')
    fig = px.line(country_over_time, x='Year', y='region', title='Participating Countries Over Time')
    st.plotly_chart(fig)
    ...
    fig = px.line(athletes_over_time, x='Year', y='Name', title='Athletes Over Time')
    st.plotly_chart(fig)
    
    st.title("No. of Events by Sport")
...
if user_menu == "Country-wise Analysis":
    ...
    fig = px.line(country_df, x="Year", y="total")
    st.header(selected_country + " Medal Tally over the years")
    st.plotly_chart(fig)

    # Excelled Sports Heatmap
